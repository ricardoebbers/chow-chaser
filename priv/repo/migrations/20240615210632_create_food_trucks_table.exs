defmodule ChowChaser.Repo.Migrations.CreateFoodTrucksTable do
  ...
  def change do
    create_if_not_exists table("food_trucks") do
    ...
    # Add a field `location` with type `geometry(Point,4326)` to the "food_trucks" table.
    # This can store a "standard GPS" (epsg4326) coordinate pair {longitude,latitude}.
    execute("SELECT AddGeometryColumn('food_trucks','location',4326,'POINT',2);")

    # Index to speed up spatial searches of the data
    create_if_not_exists index("food_trucks", [:location], using: :gist)

    # Unique index to make sure we don't have duplicate food trucks
    create_if_not_exists unique_index("food_trucks", [:object_id])
  end
end
